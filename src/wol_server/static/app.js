...
function updateStatusLed(elem) {
  fetch(`is_alive/${elem.dataset.ip}/`)
    .then((response) => response.json())
    .then((json) => elem.classList.toggle('alive', json.alive));
}
  ...
  elem.addEventListener('click', (event) => {
    ...
    fetch('wake/', {
      ...
      .then((json) => {
        ...
        statusLed = document.querySelector(
          `.status-led[data-ip="${target.ip}"]`,
        );
        setTimeout(() => updateStatusLed(statusLed), 8000);
      })
      .catch((error) => {
        console.error(error);
        toast(`!! ${error} !!`);
